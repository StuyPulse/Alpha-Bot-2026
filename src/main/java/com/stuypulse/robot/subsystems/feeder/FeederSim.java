...
public class FeederSim extends Feeder {
    ...
    public FeederSim() {
        LinearSystem<N1, N1, N1> flywheel = LinearSystemId.createFlywheelSystem(DCMotor.getKrakenX44(2), 0.20, 1.0);
    ...
    public void periodic() {
        ...
        if (Settings.EnabledSubsystems.SHOOTER.get()) {
            if (voltageOverride.isPresent()) {
                ...
                SmartDashboard.putNumber("Feeder/Input Voltage", controller.getU(0));
                sim.setInput(controller.getU(0));
            }
        } else {
            sim.setInput(0);
            SmartDashboard.putNumber("Feeder/Input Voltage", 0.0);
        }
